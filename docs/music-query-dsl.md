...
muse music-query "note.velocity > 80 and track == 'cello.mid'"
```
...
muse music-query "note.pitch_class == 'Eb'"

# Diminished chord in bar 8 specifically.
muse music-query "harmony.quality == 'dim' and bar == 8"

# High-velocity notes in the cello part authored by an agent.
muse music-query "note.velocity > 100 and track == 'cello.mid' and agent_id == 'melody-agent'"

# Notes outside a comfortable bass range.
muse music-query "note.pitch < 36 or note.pitch > 96" --track bass.mid

# Everything from a particular AI model.
muse music-query "model_id == 'claude-4'"
```

## Architecture

The DSL is implemented in three layers in `muse/plugins/music/_music_query.py`:
...
| File | Role |
|------|------|
| `muse/plugins/music/_music_query.py` | Tokenizer, parser, evaluator, `run_query` |
| `muse/cli/commands/music_query.py` | CLI command `muse music-query` |
