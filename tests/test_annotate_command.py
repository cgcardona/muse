...
def _write_commit(root: pathlib.Path, commit_id: str = "abc12345") -> CommitRecord:
    record = CommitRecord(
        commit_id=commit_id,
        repo_id="test-repo",
        branch="main",
...
class TestAnnotateCommand:
    def test_show_annotations_when_no_flags(self, repo: pathlib.Path) -> None:
        _write_commit(repo, "abc12345")
        result = runner.invoke(cli, ["annotate", "abc12345"], catch_exceptions=False)
        assert result.exit_code == 0
        assert "reviewed-by" in result.output

    def test_add_reviewer(self, repo: pathlib.Path) -> None:
        _write_commit(repo, "abc12345")
        result = runner.invoke(cli, ["annotate", "--reviewed-by", "agent-x", "abc12345"], catch_exceptions=False)
        assert result.exit_code == 0, result.output
        assert "agent-x" in result.output

        # Verify it's persisted.
        from muse.core.store import read_commit
        record = read_commit(repo, "abc12345")
    ...
    def test_add_multiple_reviewers(self, repo: pathlib.Path) -> None:
        ...
        r = runner.invoke(cli, ["annotate", "--reviewed-by", "alice,bob", "abc12345"], catch_exceptions=False)
    ...
    def test_add_reviewer_idempotent(self, repo: pathlib.Path) -> None:
        _write_commit(repo, "abc12345")
        runner.invoke(cli, ["annotate", "--reviewed-by", "alice", "abc12345"], catch_exceptions=False)
        runner.invoke(cli, ["annotate", "--reviewed-by", "alice", "abc12345"], catch_exceptions=False)
    ...
    def test_increment_test_run_counter(self, repo: pathlib.Path) -> None:
        _write_commit(repo, "abc12345")
        runner.invoke(cli, ["annotate", "--test-run", "abc12345"], catch_exceptions=False)
        runner.invoke(cli, ["annotate", "--test-run", "abc12345"], catch_exceptions=False)
